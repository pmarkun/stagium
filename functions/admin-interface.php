		...
		<script type="text/javascript">
			...
			jQuery( '#expand_options').click(function(){
				if(flip == 0){
					...
					jQuery( '#woo_container .group').add( '#woo_container .group h1').show();
					...
					jQuery( '#woo_container .group').add( '#woo_container .group h1').hide();
	...
	function woothemes_version_checker ( $local_version ) {
		...
		if( isset( $pieces_rss[0] ) && $pieces_rss[0] != 0 ) {
			...
			if ( $version_sentinel == true ) {
				$update_message = '<div class="update_available status-' . $status . '">' . __( 'Theme update is available', 'woothemes' ) . ' (v.' . $latest_version_via_rss . ') - <a href="http://www.woothemes.com/products/">' . __( 'Get the new version', 'woothemes' ) . '</a>.<p>' . sprintf( __( 'Update Type: %s', 'woothemes' ), $statuses[$status] ) . '</p></div>';
			}
			else {
				$update_message = '';
			}
...
global $pagenow;
if ( $pagenow == 'admin.php' && isset( $_GET['page'] ) && $_GET['page'] == 'woothemes' && get_option( 'framework_woo_theme_version_checker' ) == 'true' ) {
	add_action( 'admin_notices', 'woo_theme_update_notice', 10 );
} // End IF Statement
